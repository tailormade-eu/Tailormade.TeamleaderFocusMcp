...
```
{  "data": {    "type": "dealPipeline",    "id": "f350e48a-fbc3-0a79-e62a-53aa1ae86d44"  }}
```
...
```
var client = new HttpClient();var request = new HttpRequestMessage(HttpMethod.Post, "https://api.focus.teamleader.eu/dealPipelines.create");request.Headers.Add("Accept", "application/json");request.Headers.Add("Authorization", "Bearer <token>");var content = new StringContent("{\n  \"name\": \"Primary Pipeline\"\n}", null, "application/json");request.Content = content;var response = await client.SendAsync(request);response.EnsureSuccessStatusCode();Console.WriteLine(await response.Content.ReadAsStringAsync());
```
...
](/docs/api/deal-pipelines-list)[

Next

dealPipelines.update

](/docs/api/deal-pipelines-update)